...
const webdesign={
  ...
  difficulty: `average`,
  type: `science`,
  Bio: `A course that teaches you how to design websites`,
...
const math = {
  ...
  difficulty: `good`,
  type: `math`,
  Bio: `A course that teaches you how to do Algebra 2`,
  Teacher: `Mrs. OC`,
}
...
function someInfo(course) {
  console.log(`${course.name} is a fun class.`)
  return course;
}
...
function updateCourse(math, newName) {
// Modify type of course
math.name = newName;
// We'll do a console.log about the update
console.log(`The course title has been updated to ${newName}`);

};
updateCourse(math, `Knitting`);
...
function talkAboutMyCourses(anyCourse) {
  console.log(
    `${webdesign.name} is a course about ${webdesign.Bio}. The course is taught by ${webdesign.Teacher}.`
  );
}

// Create your array for #6 down here and invoke the .map() function after it.
const myCourses = [math, webdesign];
